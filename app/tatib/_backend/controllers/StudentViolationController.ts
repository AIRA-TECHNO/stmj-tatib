...
const StudentViolationController = new Elysia()
  .use(LoadUserAuthed)
  .group('/student-violation', (group) => (group.guard({ beforeHandle: guardedUserAuthed }, (app) => {
    app.get('/', async ({ request }) => {
      ...
      return await paginator(qb, new URL(request.url).searchParams, { searchableCols: ['vdu.name', 'sv.rule', 'sv.point', 'sv.note'] });
    });
