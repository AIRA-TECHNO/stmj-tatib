...
export default function Page() {
  ...
  function onExport(isMaster?: boolean) {
    ...
    api({
      url: '/tatib/api/achievement/excel/export',
      objParams: { ...(fmParams.values), ...(fmExport.values), isMaster }
    }).then(async (res) => {
      fmExport.setStatusCode(res.status);
      if (res.status == 200) downloadFile(await res.blob(), `jenis penghargaan.xlsx`);
    });
  }
  ...
  return (
    <>
      ...
      <SubMenuNav navigations={[
        { label: 'penghargaan', link: '/tatib/panel/penghargaan' },
        { label: 'jenis penghargaan', link: '/tatib/panel/jenis-penghargaan', isActive: true }
      ]} />
      <section className="sm:mt-2 max-w-7xl">
              ...
              topElements={[
                ...(isFullAccess ? [(<div className='lg:order-2 lg:ml-auto'>
                  <div onClick={() => fmDetail.setShow(true, false, true)} className='btn btn-auto-floating'>
                    ...
                    <span>jenis penghargaan baru</span>
                  </div>
                </div>)] : []),
        ...
        <Modal fm={fmDetail} noEdit={!isFullAccess} noDelete={!isFullAccess} title={<div className='mr-auto capitalize'>{fmDetail.values?.id ? 'detail' : 'tambah'} jenis penghargaan</div>}>
          <div className='px-4 pb-4'>
            <Form
              actionApi={{
                url: '/tatib/api/achievement', afterDelete: DataTable.loadDataTable, afterSubmit: (data) => {
              ...
              fm={fmDetail}
              fields={[
                { label: 'penghargaan', name: 'achievement', type: 'textarea' },
                { label: "poin", name: "point", type: 'number' },
              ]}
              noFooter
            />
          </div>
      ...
      <Modal fm={fmImport} btnClose noSubmit className='whitespace-normal max-w-xl'>
        <form className='p-4' onSubmit={(e) => onSubmitNormal(e, fmImport, {
          url: '/tatib/api/achievement/excel/import', afterSubmit: (data) => {
            toast.success(data.message);
          ...
          <InputFileDropZone
            ...
            label={<>
              <div className='mb-1'>import penghargaan</div>
              <div className='text-sm font-normal text-gray-800'>
                <span>File harus sesuai dengan format excel berikut</span>
                <a className='native-link ml-1' onClick={() => onExport(true)}>master import jenis penghargaan.xlsx</a>
              </div>
            </>}
          />
      ...
      <Modal fm={fmExport} btnClose noSubmit className='whitespace-normal max-w-xl'>
        <div className='p-4'>
          <div>
            <div className='text-lg font-semibold mb-1'>export jenis penghargaan</div>
            <div className='text-sm font-normal text-gray-800'>
