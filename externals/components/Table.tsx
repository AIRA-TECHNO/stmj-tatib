...
export default function Table({
  ...
  classNameTopElement?: string;
  topElements?: Array<ReactNode | { filter?: { show?: boolean; className?: string }, search?: { show?: boolean; className?: string }, className?: string }>;
  classNameListFilter?: string;
}) {
  ...
  return (
    <>
      <div className='bg-white pb-4'>
        <div className={cn("flex max-lg:flex-col gap-2", classNameTopElement)}>
          {topElements?.map((topElement: any, indexTopElement) => {
            ...
            if (topElement?.filter?.show || topElement?.search?.show) return (
              <div key={indexTopElement} className={cn('flex relative', topElement?.className)}>
                {Boolean(topElement?.search?.show) && (
                  <Search
                    onSubmit={handleSearch}
                    className={cn({ 'rounded-r-none': topElement?.filter?.show }, topElement?.search?.className)}
                  />
                )}
                {Boolean(topElement?.filter?.show) && (
                  <AdvanceFilter
                    fmParams={fmParams}
                    prototypeTable={prototypeTable}
                    fmFilters={fmFilters}
                    setLabelFilters={setLabelFilters}
                    className={cn({ 'rounded-l-none': topElement?.search?.show }, topElement?.filter?.className)}
                  />
                )}
              </div>
            );
        ...
        <div className={cn('flex gap-2 mt-2', classNameListFilter)}>
...
function AdvanceFilter({ prototypeTable, fmParams, fmFilters, className, setLabelFilters }: {
  prototypeTable: Array<typePrototypeTable>;
  fmParams: ReturnType<typeof useFormManager>;
  ...
  return (<>
    <div className={`btn-flat bg-primary text-contras-primary px-4 h-10 ${className}`} onClick={() => fmFilters.setShow(true)}>
    ...
    <Dropdown
      ...
      className='top-12 sm:min-w-md max-w-xl p-4 z-[5] dropdown-filter'
    >
      <div>
